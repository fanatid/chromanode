export default {
  create: {
    tables: [
      ...
      `CREATE TABLE spent (
         ...
         output_index INTEGER NOT NULL,
         output_height INTEGER,
         input_tx_id BIGINT NOT NULL REFERENCES transactions (id),
    ...
    indices: [
      ...
      `CREATE INDEX ON utxo (address_id)`,
      `CREATE INDEX ON utxo (output_tx_id)`,
      `CREATE INDEX ON utxo (output_tx_id, output_index)`,
      `CREATE INDEX ON utxo (output_height)`,
      `CREATE INDEX ON spent (output_tx_id)`,
      `CREATE INDEX ON spent (output_tx_id, output_index)`,
      `CREATE INDEX ON spent (output_height)`,
      `CREATE INDEX ON spent (input_tx_id)`,
      `CREATE INDEX ON spent (input_height)`,
  ...
  update: {
    ...
    history: {
      addConfirmedInput: `WITH output AS (
                            DELETE FROM
                              utxo
                            USING
                              transactions
                            WHERE
                              utxo.output_tx_id = transactions.id AND
                              transactions.txid = $3 AND
                              utxo.output_index = $4
                            RETURNING
                              utxo.address_id AS address_id,
                              utxo.output_tx_id AS output_tx_id,
                              utxo.output_index AS output_index,
                              utxo.output_height AS output_height
                          )
                          INSERT INTO spent SELECT *, $1, $2 FROM output`,
